...
@WebServlet("/SeekPaper")
public class SeekPaperServlet extends HttpServlet {
  @Override
  ...
  private JSONObject getAuthorLinks(String name){
    ...
    String cql1 = "match(p1:Paper)-[:`author is`]->(a1:Author) " +
        "where p1.title = $paperName " +
        "return DISTINCT  p1.title as source, a1.name as target";
    Result result1 = session.run(cql1,Values.parameters("paperName", name));
