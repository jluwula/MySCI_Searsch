...
@WebServlet("/DeleteAuthor")//这个地方记得改
public class DeleteAuthorServlet extends HttpServlet {
  ...
  private String getAuthorLinks(String name){
    ...
    try (Session session = driver.session(SessionConfig.forDatabase("sci"))) {
      String cql = "MATCH (a:Author {name: $authorName}) DETACH DELETE a";
      session.run(cql, Values.parameters("authorName", name));
      System.out.println("作者节点已成功从数据库中删除");
      session.close();
      driver.close();
      return "YES";
    } catch (Exception e) {
      System.err.println("删除作者节点时发生错误: " + e.getMessage());
      return "NO";
    }
  }
}
